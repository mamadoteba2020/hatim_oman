...
class LiquidationDependents(models.Model):
    ...
    @api.model
    def _get_default_liq_journal(self):
        param_obj = self.env['ir.config_parameter'].sudo()
        return int(param_obj.get_param('conf_dep_filtering_journal')) or False

    @api.model
    def _get_default_credit_liq_account(self):
        param_obj = self.env['ir.config_parameter'].sudo()
        return int(param_obj.get_param('conf_cre_filtering_account_2')) or False
    ...
    account_move = fields.Many2one('account.move', string='Journal Entry')
    liq_journal_id = fields.Many2one('account.journal', string='Journal', default=_get_default_liq_journal,
    ...
    date_liq = fields.Date(string='Date', required=True)
    dep_memo = fields.Char(string='Description')
    ...
    liq_partner_id = fields.Many2one('res.partner', string='Employee')
    company_id = fields.Many2one('res.company', default=lambda self: self.env.company.id)
...
class LiquidationLine(models.Model):
    ...
    inv_date_liq = fields.Date(string='Inv Date')
